...
En el caso de GraphML, el script replica los mismos atributos que Ghidra genera
cuando exportas manualmente un grafo de bloques básicos: cada nodo incluye los
campos `address`, `startAddress`, `endAddress`, `blockSize` y `label` (con las
instrucciones formateadas tal y como se muestran en la vista de grafos de
Ghidra). Además, los metadatos `functionName` y `entryPoint` se adjuntan al
grafo junto con el resto de información del programa para facilitar la
comparación con exportaciones nativas. En JSON, estos mismos datos se exponen en
la clave `ghidra_attributes` de cada bloque y arista para que puedas cruzar la
información entre ambos formatos si lo necesitas.
